...
getDataFrame <- function(df, startDate, endDate, tIDColName = "CUST_ID", tDateColName = "TRX_DATE", tAmountColName = "TRX_AMOUNT") {
...
names(cust_transact) <- c("CUST_ID", "TRX_DATE", "AMOUNT")

CDNow_df <- transact_df
# transform Date column from text to date format
CDNow_df[,2] <- as.Date( as.character(CDNow_df[,2]), "%Y%m%d")

startDate_Hist <- as.Date("19980101", "%Y%m%d")
endDate_Hist <- as.Date("19980601", "%Y%m%d")

startDate_Forecast <- as.Date("19980701", "%Y%m%d")
endDate_Forecast <- as.Date("19990930", "%Y%m%d")
